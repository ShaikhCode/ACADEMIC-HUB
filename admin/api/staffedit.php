...
function editStudent($conn)
{
    ...
    $department = $_POST['student_code'] ?? ''; // Corrected variable name
    ...
    $stmt = $conn->prepare("
        ...
        SET users.username = ?, staff.department = ?,staff.phone = ?
        WHERE staff.user_id = ?
    ");

    $stmt->bind_param("ssii", $student_name, $department,$number_p, $student_id);
